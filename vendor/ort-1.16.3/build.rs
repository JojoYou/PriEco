...
const PROTOBUF_RELEASE_BASE_URL: &str =
...
fn prebuilt_protoc_url() -> (PathBuf, String) {
    ...
    let prebuilt_url =
        format!("{PROTOBUF_RELEASE_BASE_URL}/v{PROTOBUF_VERSION}/{prebuilt_archive}");
...
fn system_strategy() -> (PathBuf, bool) {
    let lib_dir = PathBuf::from(
        env::var(ORT_ENV_SYSTEM_LIB_LOCATION)
    ...
    for i in ["Release", "Debug", "MinSizeRel", "RelWithDebInfo"] {
        if lib_dir.join(i).exists()
            && lib_dir
                .join(i)
                .join(platform_format_lib("onnxruntime_common"))
                .exists()
        {
    ...
    if lib_dir.join(platform_format_lib("onnxruntime")).exists() {
        ...
        let static_configs: Vec<(PathBuf, PathBuf, Box<dyn Fn(PathBuf, &String) -> PathBuf>)> = vec![
            (
                lib_dir.join(&profile),
                lib_dir.join("_deps"),
                Box::new(|p: PathBuf, profile| p.join(profile)),
            ),
            (
                lib_dir.clone(),
                lib_dir.parent().unwrap().join("_deps"),
                Box::new(|p: PathBuf, _| p),
            ),
            (
                lib_dir.join("onnxruntime"),
                lib_dir.join("_deps"),
                Box::new(|p: PathBuf, _| p),
            ),
        ];
                ...
                .exists()
            {
                ...
                for lib in &[
                    ...
                    if lib_path.exists() {
                        ...
                        panic!(
                            "[ort] unable to find ONNX Runtime library: {}",
                            lib_path.display()
                        );
                    }
                }
                ...
                if target_arch == "wasm32" {
                    for lib in &["webassembly", "providers_js"] {
                        let lib_path =
                            lib_dir.join(platform_format_lib(&format!("onnxruntime_{lib}")));
                        if lib_path.exists() {
                            println!("cargo:rustc-link-lib=static=onnxruntime_{lib}");
                        }
                    }
                ...
                let protobuf_build =
                    transform_dep(external_lib_dir.join("protobuf-build"), &profile);
                add_search_dir(&protobuf_build);
                for lib in ["protobuf-lited", "protobuf-lite", "protobuf"] {
                    if target_os.contains("windows")
                        && protobuf_build
                            .join(platform_format_lib(&format!("lib{lib}")))
                            .exists()
                    {
                ...
                add_search_dir(transform_dep(
                    external_lib_dir.join("google_nsync-build"),
                    &profile,
                ));
                ...
                if target_arch != "wasm32" {
                    add_search_dir(transform_dep(
                        external_lib_dir.join("pytorch_cpuinfo-build"),
                        &profile,
                    ));
                    add_search_dir(transform_dep(
                        external_lib_dir
...
fn prepare_libort_dir() -> (PathBuf, bool) {
    ...
    match strategy.as_ref().map_or("download", String::as_str) {
        ...
        "download" => {
            if target.contains("darwin") {
                incompatible_providers![
                    CUDA, ONEDNN, OPENVINO, VITIS, TVM, TENSORRT, MIGRAPHX, DIRECTML, WINML, ACML,
                    ARMNN, ROCM
                ];
            } else {
                incompatible_providers![
                    ONEDNN, COREML, OPENVINO, VITIS, TVM, MIGRAPHX, DIRECTML, WINML, ACML, ARMNN,
                    ROCM
                ];
            }
        ...
        "compile" => {
                ...
                |_| {
                        ...
                        && target.contains("linux")
                    {
                        root.join("toolchains")
                            .join("default-aarch64-linux-gnu.cmake")
                    } else if cfg!(target_os = "linux")
                        && target.contains("aarch64")
                        && target.contains("windows")
                    {
                        root.join("toolchains")
                            .join("default-aarch64-w64-mingw32.cmake")
                    } else if cfg!(target_os = "linux")
                        && target.contains("x86_64")
                        && target.contains("windows")
                    {
                        root.join("toolchains")
                            .join("default-x86_64-w64-mingw32.cmake")
                    } else {
                        PathBuf::default()
                    }
                },
            ...
            #[cfg(windows)]
            {
                ...
                match vs_find_result {
                    ...
                    | None => panic!("[ort] unable to find Visual Studio installation"),
                };
            }
            ...
            println!(
                "cargo:rustc-link-search=native={}",
                external_lib_dir.join("protobuf").join("cmake").display()
            );
            println!("cargo:rustc-link-lib=static=protobuf-lited");

            println!(
                "cargo:rustc-link-search=native={}",
                external_lib_dir.join("onnx").display()
            );
            println!("cargo:rustc-link-lib=static=onnx");
            println!("cargo:rustc-link-lib=static=onnx_proto");
            ...
            println!(
                "cargo:rustc-link-search=native={}",
                external_lib_dir.join("nsync").display()
            );
            println!("cargo:rustc-link-lib=static=nsync_cpp");

            println!(
                "cargo:rustc-link-search=native={}",
                external_lib_dir.join("re2").display()
            );
            println!("cargo:rustc-link-lib=static=re2");
        ...
        _ => panic!(
            "[ort] unknown strategy: {} (valid options are `download` or `system`)",
            strategy.unwrap_or_else(|_| "unknown".to_string())
        ),
    }
}
...
fn real_main(link: bool) {
    ...
    let (include_dir, lib_dir) =
        if install_dir.join("include").exists() && install_dir.join("lib").exists() {
    ...
    if std::env::var("DOCS_RS").is_err() {
        if cfg!(feature = "load-dynamic") {
            ...
            if cfg!(feature = "download-binaries")
                && std::env::var(ORT_ENV_STRATEGY)
                    .as_ref()
                    .map_or("download", String::as_str)
                    == "download"
            {
