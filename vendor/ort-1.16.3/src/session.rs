...
use super::{
    ...
    error::{
        assert_non_null_pointer, assert_null_pointer, status_to_result, OrtApiError, OrtError,
        OrtResult,
    },
...
impl SessionBuilder {
    ...
    pub fn with_execution_providers(
        mut self,
        execution_providers: impl AsRef<[ExecutionProvider]>,
    ) -> OrtResult<Self> {
        ...
        P: AsRef<Path> + std::fmt::Debug,
    {
        ...
        if model_filepath.exists() {
            ...
            let bytes_io_count =
                std::io::copy(&mut reader, &mut writer).map_err(OrtDownloadError::IoError)?;
        ...
        P: AsRef<Path>,
    {
        ...
        let initializer_names: Vec<CString> = initializers
            ...
            .collect();
        let initializer_names_ptr: Vec<*const c_char> = initializer_names
            .iter()
            .map(|n| n.as_ptr() as *const c_char)
            .collect();

        let initializers: Vec<*const sys::OrtValue> = initializers
            .iter()
            .map(|input_array_ort| input_array_ort.1.ptr() as *const _)
            .collect();
...
impl Session {
    ...
    pub fn bind(&self) -> OrtResult<IoBinding<'_>> {
        IoBinding::new(self)
    }
    ...
    pub fn run<'s, 'm, 'v, 'i>(
        &'s self,
        input_values: Vec<Value<'v>>,
    ) -> OrtResult<Vec<Value<'static>>>
    where
        ...
        'i: 'v,
    {
        ...
        let input_ort_values: Vec<*const sys::OrtValue> = input_values
            .iter()
            .map(|input_array_ort| input_array_ort.ptr() as *const _)
            .collect();
...
mod dangerous {
    ...
    fn extract_io_count(
        ...
        (num_nodes != 0).then_some(()).ok_or_else(|| {
            OrtError::GetInOutCount(OrtApiError::Msg("No nodes in model".to_owned()))
        })?;
        Ok(num_nodes as _)
    }
    ...
    pub(crate) fn raw_pointer_to_string(
        allocator_ptr: *mut sys::OrtAllocator,
        c_str: *mut c_char,
    ) -> OrtResult<String> {
