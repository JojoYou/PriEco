...
$tmpC = $loc;
if($tmpC==null or $tmpC == 'all'){
$tmpC = 'us';
}
if(!$dev) {
    //API keys ($_ENV variables loaded in database.php file) 
    $Googlefile = 'https://www.googleapis.com/customsearch/v1?key='.$_ENV['GOOGLE_API_KEY'].'&cx='.$_ENV['GOOGLE_CX_KEY'].'&hl='.$lang.'&gl='.$loc.'&dateRestrict='.$date.'&safe='.$safe.'&q='.$Bpurl;
    $BraveFile = 'https://api.search.brave.com/res/v1/web/search?spellcheck=0&result_filter=web&country='.$tmpC.'&search_lang='.$tmp;if($safe!='active'){$BraveFile .= '&safesearch=off';}$BraveFile.='&freshness=';if($date!=''){$BraveFile .= 'p'.str_replace('1','',$date);}$BraveFile.='&q=' . $Bpurl;
    $MojeekFile = 'https://www.mojeek.com/search?api_key='.$_ENV['MOJEEK_API_KEY'].'&lb='.$lang.'&lbb=50'.'&rb='.$loc.'&rbb=50&fmt=json&q='.$Bpurl;
    ...
    $PixabayFile = 'https://pixabay.com/api/?key='.$_ENV['PIXABAY_API_KEY'].'&per_page=200&q='.$Bpurl;

    $NewsFile = 'https://newsapi.org/v2/everything?apiKey='.$_ENV['NEWS_API_KEY'].'&from='.date('Y-m-d', $newsdate).'&sortBy=popularity&language='.$tmp.'&q='.$Bpurl;
    $YoutubeFile = 'https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=50&key='.$_ENV['YOUTUBE_API_KEY'].'&q='.$Bpurl;
}
else{
    //Null API keys    
